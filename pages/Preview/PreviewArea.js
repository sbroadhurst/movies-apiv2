...
class PreviewArea extends React.Component {
      ...
      return (
        <div>
          {movie.genres.map((info, index) => {
            return (
              <Link key={index} href={{ pathname: '/genre/' + info.name + '/' + info.id }}>
                <RaisedButton primary={true} className="tags" label={info.name} info={info}>
                  {' '}
                </RaisedButton>
              </Link>
            )
  ...
  trailer() {
    ...
    if (movie.videos && movie.videos.results.length > 0) {
      ...
      var trailerUrl = 'https://www.youtube.com/embed/' + movie.videos.results[0].key
      return <iframe width="420" height="315" src={trailerUrl} title="trailer" frameBorder="0" allowFullScreen />
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div>
          <div>
            <div className="main-page-text">Similar {this.props.match.params.type}</div>
            <br />
          </div>
