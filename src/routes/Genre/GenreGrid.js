...
class GenreGrid extends React.Component {
  ...
  loadGenreTv() {
    ...
    fetch(
      'https://api.themoviedb.org/3/discover/tv?api_key=4d6322cf6a2b7554c7e6ffbaec593010&language=en-US&sort_by=popularity.desc&page=1&with_genres=' +
        id
    )
      .then(res => {
        return res.json()
  ...
  LoadGenreMovies() {
    ...
    fetch(
      ' https://api.themoviedb.org/3/genre/' +
        id +
        '/movies?api_key=4d6322cf6a2b7554c7e6ffbaec593010&language=en-US&include_adult=false&sort_by=created_at.asc'
    )
      .then(res => {
        return res.json()
      })
      ...
      return (
        <div>
          <div className="main-page-text">
            {' '}
            {this.props.match.params.genre} Movies
          </div>
          <br />
  ...
  mapTV(GenreTv) {
    if (GenreTv !== undefined)
      return (
        <div>
          <div className="main-page-text">
            {this.props.match.params.genre} TV Shows
          </div>
          <br />
  ...
  render() {
    ...
    return (
      <div>
        {' '}
        {this.mapMovies(GenreMovies)} {this.mapTV(GenreTv)}
      </div>
    )
  }
}
